...
import { addEmailAccount, listEmailAccounts } from './emailAccounts';
...
router.get("/auth/gmail", authenticate, (req, res) => {
  try {
    ...
    const url = oauth2Client.generateAuthUrl({
      ...
      scope: scopes,
      prompt: "consent",
      state: req.user.uid // Include user ID in state for callback
    });
...
router.get("/auth/gmail/callback", async (req, res) => {
  ...
  if (!userId) return res.status(400).send("Missing user ID in callback");
...
router.get("/gmail/accounts", authenticate, async (req, res) => {
  try {
    console.log(`Fetching Gmail accounts for user: ${req.user.uid}`);
    
    const accounts = await listEmailAccounts(req.user.uid);

    console.log(`Found ${accounts.length} Gmail accounts for user ${req.user.uid}`);
    res.json(accounts);
  } catch (error) {
...
router.get("/gmail/emails", authenticate, async (req: Request & { user?: { uid: string } }, res) => {
  try {
