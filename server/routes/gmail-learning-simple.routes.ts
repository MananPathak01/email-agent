...
import {CommunicationProfileService, EmailData} from '../services/communication-profile.service.js';
...
gmailLearningSimpleRouter.get('/learning-progress', authenticate, async (req, res) => {
    try {
        ...
        const progress = global.learningProgress ?. [userId] || {
    ...
    async function performSimpleEmailLearning(userId : string, specificEmail? : string, userContext? : any) {
        try {
            ...
            global.learningProgress[userId] = {
                stage: 'collecting',
                message: 'Setting up Gmail connection...',
                progress: 20
            };
            ...
            if (! tokens) 
                throw new Error(`No Gmail tokens found for ${
                    specificEmail || 'user'
                }`);
            ...
            global.learningProgress[userId] = {
                stage: 'collecting',
                message: 'Preparing writing-style data...',
                progress: 55
            };
            ...
            try {
                ...
                if (files.length) {
                    ...
                    // Slim for prompt
                    const slimSamples = Array.isArray(json.sentEmails) ? json.sentEmails.slice(0, 5).map((e
                    : any) => ({from: e.from, to: e.to, subject: e.subject, wordCount: e.wordCount})) : [];
                    writingStyleData = {
                        userId: json.userId,
                        email: json.email,
            ...
            } catch (e) { // If any error occurs reading existing file, we'll build fresh
            ...
            if (! writingStyleData) {
                global.learningProgress[userId] = {
                    stage: 'collecting',
                    message: 'Collecting sent emails (compact)...',
                    progress: 60
                };
                ...
                const compactSamples = sentEmailsSample.filter((e : any) => e && e.subject).slice(0, 5).map((e : any) => ({
                    from: e.from || 'unknown',
                ...
                try {
                    ...
                    const fileName = `writing-style-data-${emailSlug}-${
    ...
    async function addThreadContextToEmails(gmail : any, sentEmails : any[]) {
        const emailsWithContext = await Promise.all(sentEmails.map(async (email) => {
            ...
            try {
                ...
                const contextEmails = threadMessages.filter((msg : any) => msg.id !== email.id).map((msg : any) => {
                ...
                return {
                    ...email,
                    threadContext: contextEmails.filter(
                        (ctx : any) => ctx !== null
                    )
                };
            } catch (error) {
