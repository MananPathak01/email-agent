...
export class GmailWatchService {
  static getTopicName(): string {
    ...
    if (!topic) throw new Error('PUBSUB_TOPIC_GMAIL env is required');
    // Expected format: projects/<project-id>/topics/<topic-name>
    if (!topic.startsWith('projects/')) throw new Error('PUBSUB_TOPIC_GMAIL must be a fully qualified Pub/Sub topic name');
    return topic;
  }

  static async registerWatchForAccount(userId: string, accountEmail: string): Promise<{ historyId: string; expiration?: string } | null> {
  ...
  static async updateAccountWatchState(userId: string, accountEmail: string, data: any) {
