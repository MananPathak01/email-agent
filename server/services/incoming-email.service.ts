...
export class IncomingEmailService {
  /** Process a Pub/Sub notification for a given email account. */
  static async processNotification(emailAddress: string, notificationHistoryId: string | number) {
    ...
    const accountSnap = await adminDb
      .collectionGroup('email_accounts')
    ...
    do {
      ...
      const hist = res.data;
      if (hist.historyId) latestHistoryId = String(hist.historyId);
      if (hist.history) {
    ...
    for (const messageId of messageIds) {
      ...
      if (processedSnap.exists) continue; // idempotent

      // Fetch message metadata
      const msgRes = await gmail.users.messages.get({ userId: 'me', id: messageId, format: 'metadata', metadataHeaders: ['From','To','Subject','Message-Id','References','Reply-To','List-Unsubscribe','Precedence'] });
      ...
      try {
        ...
        await processedRef.set({ status: 'processed', replyDraftId: draftId, subject: getHeader(headers, 'Subject'), from, threadId: message.threadId, createdAt: new Date().toISOString() });
      } catch (e:any) {
        console.error('[Incoming] Failed to create draft:', e?.message || e);
        await processedRef.set({ status: 'error', error: e?.message || String(e), createdAt: new Date().toISOString() });
      }
    }
