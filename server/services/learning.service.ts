...
import { doc, getDoc, setDoc, collection, query, where, getDocs, limit, orderBy, addDoc } from 'firebase/firestore';
...
export async function getSimilarResponses(userId: string, emailAnalysis: any) {
  try {
    ...
    const q = query(
      collection(db, 'email_analysis'),
      where('userId', '==', userId),
      where('analysis.category', '==', emailAnalysis.category),
...
export async function processHistoricalEmails(userId: string, accountId: string, emailData: any[]) {
  try {
    ...
    await setDoc(
      doc(db, 'user_communication_styles', userId),
      communicationStyle
    );
...
export async function updateLearningFromFeedback(userId: string, emailId: string, originalDraft: string, userEdit: string) {
  try {
    ...
    await setDoc(
      doc(db, 'user_feedback', `${userId}_${emailId}_${Date.now()}`),
      feedback
    );
