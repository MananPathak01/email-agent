...
export class VectorSearchService { // Calculate cosine similarity between two vectors
    ...
    async findSimilarEmails(queryText : string, existingEmails : Array < {
        id: number;
        embedding: string;
        [key: string]: any
    } >, threshold : number = 0.7, limit : number = 5): Promise < SimilarityResult[] > {
        try { // Generate embedding for the query text
            ...
            for (const email of existingEmails) {
                ...
                try { // Parse the stored embedding
                    ...
                    if (similarity >= threshold) {
                        similarities.push({id: email.id, similarity, email});
                    }
                } catch (error) {
                    console.error(`Error processing embedding for email ${
                        email.id
                    }:`, error);
                    continue;
                }
    ...
    async findSimilarResponsePatterns(emailContent : string, category : string, existingEmails : Array < {
        id: number;
        embedding: string;
        category: string;
        [key: string]: any
    } >, threshold : number = 0.6): Promise < SimilarityResult[] > {
        try { // Filter emails by category first
            const categoryEmails = existingEmails.filter(email => email.category === category && email.embedding);
    ...
    async generateAndStoreEmbedding(emailContent : string, subject : string): Promise < string > {
        try { // Combine subject and content for better context
    ...
    async findRelatedTasks(emailContent : string, existingTasks : Array < {
        id: number;
        title: string;
        description: string;
        [key: string]: any
    } >, threshold : number = 0.6): Promise < Array < {
        taskId: number;
        similarity: number;
        task: any
    } >> {
        try {
            ...
            for (const task of existingTasks) { // Combine task title and description for comparison
                const taskText = `${
                    task.title
                }\n${
                    task.description || ''
                ...
                try {
                    ...
                    if (similarity >= threshold) {
                        results.push({taskId: task.id, similarity, task});
                    }
                } catch (error) {
